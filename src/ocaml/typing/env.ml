...
(* let prune_usage_tables ~stamp =
  (* We remove *)
  let prune tbl =
    Types.Uid.Tbl.filter_map_inplace
      (fun uid value ->
        if Shape.Uid.is_more_recent ~stamp uid then None else Some value)
      tbl
  in
  prune !value_declarations;
  prune !type_declarations;
  prune !module_declarations *)
